...
For any additional areas predicted by the model above the actual amount of ground-truth components, a penalty is computed:
\\
$$
...
This paper presents several complete pipelines for CMFD on the Recod.AI Scientific Image Forgery Detection dataset. The methods are all fundamentally based on  Meta AI's DINOv2 segmentation features using the pretrained \emph{ViT-B/14} segmentation head \cites{oquab2023dinov2}, where the baseline model passes the features directly through a decoder. To build on this, we also present more complex architectures based on BusterNet \cites{wu2018busternet} and Cross-Scale PatchMatch \cites{li2024deeppatchmatch}. The strengths and weaknesses of these approaches are discussed, providing an empirical basis for comparing different design choices. Through this analysis, the paper identifies directions that appear promising for future work and further nuances the task of CMFD.
\section{Methods}
...
The modern implementation of BusterNet \cite{wu2018busternet} uses the core concepts of a similarity layer, a two-branch approach with a fusion model, and a 3 stage training setup.  Wu \etal emphasize that the core concepts of BusterNet are most central and that inner implementations could be changed by the reader. This inspired our choices to adapt the inner architecture to work with the DINOv2 model as a feature extractor, and to utilize modern deep learning principles for efficiency and robustness. For more implementation details, see GitHub \cite{einar_busternet}.
...
BusterNet-DINO achieved the strongest holdout oF1 among the three architectures. Results are summarised in Table~\ref{tab:busternet_results}.
\begin{table}[H]
	\centering
	\small
	\begin{tabular}{lc}
		...
		\bottomrule
	\end{tabular}
	\caption{BusterNet-DINO results. $^*$Non-empty metrics computed on validation forged images with a non-empty prediction.}
...
\printbibliography
\appendix
\section{BusterNet-DINO Diagnostics}

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{busternet_boxplot.png}
	\caption{BusterNet-DINO validation distributions. Left to right: maximum predicted probability per image (authentic vs.\ forged), 99th-percentile probability, total predicted pixels, and pixel F1 by ground-truth mask size bucket. Forged images show bimodal behaviour — the model either fires confidently or produces an empty prediction. F1 degrades for smaller forgeries, consistent with the size-bucket summary in the main text.}
...
\begin{figure}[H]
	...
	\caption{BusterNet-DINO prediction on a nanoparticle image (case 22821, F1=0.526). The model identifies the copied rectangular region; residual errors at the edges are visible in the error overlay. This example illustrates the challenge of precise boundary localisation on high-contrast scientific images.}
	\label{fig:busternet_success_2}
\end{figure}

\section{PatchMatch Post-Processing Performance Gap}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{patchmatch_good_example_no_postprocess.png}
	\caption{PatchMatch on an image using no post-processing, achieving oF1=0.063. The model detects the copied object but also generates many small components. This affects the oF1 penalty $P$, leading to an overall low oF1 score, even though per-pixel precision is quite strong.}
...
\begin{figure}[H]
	...
	\caption{PatchMatch on the same figure using post-processing, achieving oF1=0.948}
...
\begin{figure}[H]
	...
	\caption{An authentic image containing very similar objects, causing false positives in the Cross-Scale PatchMatch prediction.}
	\label{fig:false_positive_auth}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{hard_prediction.png}
	\caption{A forged image with the source being hard to predict due to size. The aggressive minimum component size post-processing performed by some of the models examined in this paper, such as Cross-Scale PatchMatch, mean that they would be unable to detect this forgery.}
