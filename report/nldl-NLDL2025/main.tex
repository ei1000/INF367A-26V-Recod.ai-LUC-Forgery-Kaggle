...
\\\\
Our initial implementation was inspired by a notebook by Kaggle user Gaurav Parkhedkar \cites{notebook}, who successfully employed a range of different techniques to perform a score of $0.332$ on the competition metric on the test set. These techniques include feature extraction through the DINOv2 model \cites{oquab2023dinov2}, sliding window inference and prediction mask post-processing.
\subsection{Dataset}
...
\\\\
Available memory is also a major limitation, as the operations are performed concurrently on three different scalings of the image. Running on a NVIDIA RTX 4070 TI SUPER, feature extraction and prediction on each batch was performed in less than $0.1$s, while the PatchMatch module and iterations took up to $7.3$s per batch when employed with 24 iterations. Because of memory limits, we were not able to employ the same DINOv2 backbone model as in the main pipeline, instead opting for the lighter VGG16 weights from \lstinline|torchvision.models| \cites{simonyan2014vgg16}. 
\\\\
An interesting further research direction would be to combine some techniques from our main pipeline and the PatchMatch module. For instance, running the PatchMatch architecture in a single-scale manner would save memory resources, perhaps enabling the use of a stronger DINOv2 backbone which could facilitate more powerful pixel-level features.
\\\\
TODO: Fill in table 
\begin{table}[h!]
	\centering
