...
\subsection{Modernized BusterNet implementation}
The modern implementation of BusterNet \cite{wu2018busternet}
uses the core concepts of a similarity layer,
a two-branch approach with a fusion model, and a 3 stage training setup.
The inner architecture was adapted to work with the baseline's DINOv2 as a feature extractor
and modern deep learning principles for efficiency and robustness.
The paper made it explicit that the core concepts were most important and inner implementations could be changed by the reader.
Therefore it was most fair to revise it to be comparable with the baseline implementation too using modern decisions.
For more implementation details check the implementation \cite{einar_busternet}
